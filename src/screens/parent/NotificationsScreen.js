...
const NOTIF_ICONS = {
  GRADE:      { icon: 'bar-chart-outline',        color: '#059669', bg: '#D1FAE5' },
  PAYMENT:    { icon: 'wallet-outline',            color: '#0891B2', bg: '#E0F2FE' },
  ATTENDANCE: { icon: 'checkbox-outline',          color: '#D97706', bg: '#FEF3C7' },
...
// access / évaluation access for the child) — surfaced in a distinct
// red/white card above everything else instead of blending into the
...
export default function ParentNotificationsScreen({ navigation }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.log('Parent notifications error:', e.message);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const renderPriorityNotif = (item) => {
    ...
    return (
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.priorityMessage}>{item.message || item.body || ''}</Text>
        </View>
          ...
          ListHeaderComponent={
            priorityNotifs.length > 0 ? (
              <View style={styles.group}>
                {priorityNotifs.map(renderPriorityNotif)}
              </View>
            ) : null
          }
          ListEmptyComponent={
...
const styles = StyleSheet.create({
  ...
  notifCardUnread: { borderLeftWidth: 3, borderLeftColor: colors.success },
  ...
  unreadDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: colors.success, marginTop: 6, flexShrink: 0 },
